...
control <-read.table(args[2],header=FALSE)
cc = as.numeric(args[3])
cm = as.numeric(args[4])
mb = as.numeric(args[5])

subject<- subset(subject,  V5 > mb)
control<-subset(control,  V5 > mb)

#subject$V7 <- (subject$V4/(cc/2)) - (control$V4/ (cm/2)) 
#subject$V9 <- subject$V4
#subject$V10 <- control$V4
#subset(subject, V6 >=1)
path<-paste0(trimws(args[1]), ".VS.")
path<-paste0(path, trimws(args[2]))
...
for(ch in chroms) {
        print(  ggplot()+
                geom_point(aes(x=subset(control, V1 == ch)$V2, y=subset(control, V1 == ch)$V4/(cm/2)), color="green3", alpha =0.5)+
                geom_point(aes(x=subset(subject , V1 == ch)$V2, y=subset(subject, V1 == ch)$V4/(cc/2)), color="firebrick3", alpha = 0.5)+
